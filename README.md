...
EsoKR이 처음 만들어질 당시에는 BackupFont 체이닝도, slugfont.exe를 통한 커스텀 폰트 생성도 불가능했습니다. 중국어 폰트 경로에 한글을 매핑하는 CJK 우회가 한글을 표시할 수 있는 사실상 유일한 방법이었고, 이 기반 패치가 있었기에 ESO 한글 커뮤니티가 유지될 수 있었습니다.

TamrielKR은 이후 추가된 BackupFont 체이닝(2020)과 Slug 폰트 시스템(2024) 덕분에 가능해진 방식입니다. 네이티브 UTF-8 한글을 그대로 사용하면서, `GetCVar` 후킹으로 다른 애드온에는 영문 클라(`"en"`)로 인식시킵니다. 덕분에 **한글 클라 상태에서 TTC 등 영문 애드온이 정상 동작**하고, 영문 애드온의 한글 번역 패치도 가능해졌습니다.
