...
local function Initialize(eventCode, addOnName)
	...
	local actualLanguage = TamrielKR and type(TamrielKR.GetLanguage) == "function" and tostring(TamrielKR:GetLanguage()) or BUI.language
	if actualLanguage and BUI.Localization[actualLanguage] then
		BUI.language = actualLanguage
	end
...
--	ZO_Synergy.container:SetAnchor(BOTTOM, nil, BOTTOM, 0, -230)
	...
	SLASH_COMMANDS["/rl"]=function() BUI.OnScreen.Notification(8,BUI.Loc("SETTINGS_ReloadingUI")) BUI.CallLater("ReloadUI",1000,ReloadUI) end	--SLASH_COMMANDS["/reloadui"]
