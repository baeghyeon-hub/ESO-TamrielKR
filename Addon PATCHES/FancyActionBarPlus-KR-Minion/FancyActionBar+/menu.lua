...
local KR_MENU = nil
do
    local krLocale = rawget(_G, "FancyActionBarKR")
    if krLocale and type(krLocale.menu) == "table" then
        KR_MENU = krLocale.menu
    end
end

local KR_MENU_REVERSE = nil
if KR_MENU then
    KR_MENU_REVERSE = {}
    for sourceText, translatedText in pairs(KR_MENU) do
        if type(sourceText) == "string" and type(translatedText) == "string" and KR_MENU_REVERSE[translatedText] == nil then
            KR_MENU_REVERSE[translatedText] = sourceText
        end
    end
end

local LOCALIZABLE_OPTION_KEYS =
{
    name = true,
    displayName = true,
    title = true,
...
local function Tr(text)
    if KR_MENU and type(text) == "string" then
        return KR_MENU[text] or text
    end
    return text
end

local function Untr(text)
    if KR_MENU_REVERSE and type(text) == "string" then
        return KR_MENU_REVERSE[text] or text
    end
    return text
end

local function LocalizeChoices(choices)
    if type(choices) ~= "table" then
        return choices
    end

    local localizedChoices = {}
    local changed = false

    for index, choice in ipairs(choices) do
        if type(choice) == "string" then
            localizedChoices[index] = Tr(choice)
            changed = changed or localizedChoices[index] ~= choice
        else
            localizedChoices[index] = choice
        end
    end
...
local function WrapLocalizedChoices(option)
    if not KR_MENU or option._fabLocalizedChoices then
        return
    end

    local originalChoices = option.choices
    if type(originalChoices) == "table" then
        option.choices = LocalizeChoices(originalChoices)
    elseif type(originalChoices) == "function" then
        option.choices = function(...)
            return LocalizeChoices(originalChoices(...))
        end
    else
        return
    end

    local originalGet = option.getFunc
    ...
    option._fabLocalizedChoices = true
end

local function LocalizeLAMTable(data)
    if not KR_MENU or type(data) ~= "table" then
        return data
    end

    for key, value in pairs(data) do
        if LOCALIZABLE_OPTION_KEYS[key] and type(value) == "string" then
            data[key] = Tr(value)
        end
    end

    if data.choices ~= nil then
        WrapLocalizedChoices(data)
    end

    if type(data.controls) == "table" then
        for _, control in ipairs(data.controls) do
            LocalizeLAMTable(control)
        end
    end
...
local function SetSkillToBlacklistID(id, blacklistConfigData)
    ...
    if not isValid then
        CHAT_ROUTER:AddSystemMessage("|cffffff" .. id .. Tr(" is not a valid ID."))
    ...
    if tonumber(id) then
        ...
        CHAT_ROUTER:AddSystemMessage(Tr("Skill to blacklist updated to: ") .. blacklistConfigData.skillToBlacklistName)
        if not IsConsoleUI() then
...
local function SetSelectedBlacklist(string, blacklist, blacklistConfigData)
    if string == "== Select a Skill ==" or string == Tr("== Select a Skill ==") or string == "" or string == nil then
        return
    end
    if blacklistConfigData.blacklistedSkillNames[string] then
        ...
        if blacklist[id] then
            ...
            CHAT_ROUTER:AddSystemMessage(Tr("selected: ") .. string)
        end
    end
end
...
local function GetBlacklistedSkills(blacklist)
    local list = {}
    local default = Tr("== Select a Skill ==")
    table.insert(list, default)

    for id, _ in pairs(blacklist) do
        local name = GetAbilityName(id) or Tr("Unknown Ability")
        local nameString = name .. " (" .. id .. ")"
...
local function BlacklistId(blacklist, blacklistConfigData)
    if CanBlacklistId(blacklist, blacklistConfigData) then
        ...
        CHAT_ROUTER:AddSystemMessage(Tr("Failed to blacklist: ") .. blacklistConfigData.skillToBlacklistName)
    end
end
...
local function SetSkillToBlacklistID(id)
  ...
  if tonumber(id) then
    ...
    CHAT_ROUTER:AddSystemMessage(Tr('Skill to blacklist updated to: ') .. skillToBlacklistName .. ' (' .. skillToBlacklistID .. ')' )
...
local function SetSelectedBlacklist(string)
  ...
  if string == '== Select a Skill ==' or string == Tr('== Select a Skill ==') or string == '' or string == nil then return end
  if blacklistedSkillNames[string] then
    ...
    if SV.externalBlackList[id] then
      ...
      CHAT_ROUTER:AddSystemMessage(Tr('selected: ') .. string .. ' (' .. selectedBlacklist .. ')')
    end
  end
end
...
local function BlacklistId()
  if CanBlacklistId() then
    ...
    CHAT_ROUTER:AddSystemMessage(Tr('Failed to blacklist: ') .. skillToBlacklistName .. ' (' .. skillToBlacklistID .. ')' )
  end
end
            ...
            if craftedId ~= 0 then
                ...
                if cfg[2] then
                    for scriptKey, effect in pairs(cfg[2]) do
                        ...
                        elseif effect == false then
                            scribedStr = scribedStr .. " (" .. Tr("Disabled") .. ")"
                        else
                            scribedStr = scribedStr .. " (" .. Tr("Not Tracked") .. ")"
                        end
...
local function SetSkillToEditID(id)
    ...
    if isCrafted then
        ...
        CHAT_ROUTER:AddSystemMessage(Tr("Skill to edit updated to: ") .. skillToEditName .. scriptsStr .. " (" .. skillToEditID .. ")")
        if not IsConsoleUI() then
        ...
        CHAT_ROUTER:AddSystemMessage(Tr("Skill to edit updated to: ") .. skillToEditName .. " (" .. skillToEditID .. ")")
        if not IsConsoleUI() then
            WM:GetControlByName("SkillToEditTitle").desc:SetText(skillToEditID)
        end
    end
...
local function SetEffectToTrackID(id)
    ...
    if tonumber(id) then
        ...
        CHAT_ROUTER:AddSystemMessage(Tr("Effect to track updated to: ") .. effectToTrackName .. " (" .. effectToTrackID .. ")")
        if not IsConsoleUI() then
...
local function SetChangedSkillToEdit(string)
    if string == nil or string == "" or string == "== Select a Skill ==" or string == Tr("== Select a Skill ==") then
        return
    end
...
local function FormatSkillUpdateMessage()
    ...
    if tonumber(newEffect) > 0 then
        message = zo_strformat(Tr("Updating |cffa31a<<1>>|r (|cffffff<<2>>|r) effect to: |cffa31a<<3>>|r (|cffffff<<4>>|r)"), skillToEditName, skillToEditID, newEffectName, newEffect)
    else
        message = zo_strformat(Tr("Tracking of |cffa31a<<1>>|r is now disabled."), skillToEditName)
    end
    if craftedId then
        ResetCraftedAbilityScriptSelectionOverride()
    end
...
local function ValidateSkillChange()
    ...
    if valid then
        ...
        CHAT_ROUTER:AddSystemMessage(Tr("Failed to update effect"))
    end
end
    ...
    local function BuildOptions()
        ...
        table.insert(optionsTable,
            {
                ...
                controls =
                {
                    ...
                    {
                        type = "description",
                        title = Tr("[ |cffdf80Enemy Markers|r ]"),
                        text = Tr("This replicates a similar feature found in the Untaunted addon."),
                        width = "full",
                    },
                    {
                        type = "checkbox",
                        name = Tr("Show Enemy Markers"),
                        tooltip = Tr("Display a red arrow over the head of enemies you are currently in combat with."),
                    ...
                    {
                        type = "submenu",
                        name = Tr("|cFFFACDGlobal Cooldown Tracker|r"),
                        controls =
                        {
                            {
                                type = "description",
                                title = Tr("[ |cffdf80Adjust GCD Size|r ]"),
                                width = "full",
                            },
                            {
                                type = "checkbox",
        ...
        if IsConsoleUI() then
            local moveGCD =
            {
                {
                    type = "description",
                    title = Tr("[ |cffdf80Adjust GCD Position|r ]"),
                    width = "full",
                },
                {
                    type = "slider",
                    name = Tr("Horizontal (X) Position"),
