...
function addon:InitPinSizes()
	...
	local settings = LibHarvensAddonSettings:AddAddon(GetString(SI_VOTANSMINIMAP_PINSIZES_PANEL_TITLE))
	...
	local function AddCustomPin(pinType, caption, stringId)
		if not pinType then return end
		AddPin(pinType, caption, stringId or _G["SI_MAPFILTER" .. pinType])
	end
	...
	if HarvensCustomMapPinsType then
		AddCustomPin(HarvensCustomMapPinsType, "Harvens Custom Map Pins", SI_VOTANSMINIMAP_ADDPINS_HARVENS_CUSTOM)
	end
	...
	if LostTreasureMapTreasurePin then
		...
		AddAddonPins(mapPins, "Lost Treasure", SI_VOTANSMINIMAP_ADDPINS_LOST_TREASURE)
	end
	...
	if pinType_Chronic_Chronogler then
		...
		AddAddonPins(mapPins, "Map Pins", SI_VOTANSMINIMAP_ADDPINS_MAP_PINS)
	end
	...
	AddCustomPin(VotansFishermanPinType, "Votan's Fisherman Pins", SI_VOTANSMINIMAP_ADDPINS_FISHERMAN)
