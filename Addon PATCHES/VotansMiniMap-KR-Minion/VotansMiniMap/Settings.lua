...
addon.fontFaces = {
	["MEDIUM_FONT"] = {"EsoKR/fonts/univers57.otf", 1},
	["BOLD_FONT"] = {"EsoKR/fonts/univers67.otf", 1},
	["CHAT_FONT"] = {"EsoKR/fonts/univers67.otf", 1},
...
function addon:InitMapSettings()
	...
	if type(self.account.titleFontSize) == "string" then
		CreateFont("VOTAN_MINIMAP_FONT", "EsoKR/fonts/univers57.otf|" .. self.account.titleFontSize)
...
function addon:InitSettings()
	...
	local settings = LibHarvensAddonSettings:AddAddon(GetString(SI_VOTANSMINIMAP_PANEL_TITLE))
	...
	if self.account.enableMap then
		...
		do
			local items = {
				{name = GetString(SI_VOTANSMINIMAP_TITLE_POSITION_TOP), data = true},
				{name = GetString(SI_VOTANSMINIMAP_TITLE_POSITION_BOTTOM), data = false}
			}
			settings:AddSetting {
				type = LibHarvensAddonSettings.ST_DROPDOWN,
		...
		do
			local Modes = {
				{name = GetString(SI_VOTANSMINIMAP_TIME_FORMAT_12), data = TIME_FORMAT_PRECISION_TWELVE_HOUR},
				{name = GetString(SI_VOTANSMINIMAP_TIME_FORMAT_24), data = TIME_FORMAT_PRECISION_TWENTY_FOUR_HOUR}
			}
			local ModeToData = {}
			for i = 1, #Modes do
